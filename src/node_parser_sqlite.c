...
void on_node_parser_start_sqlite(const struct node_parser* node_parser) {
    MEM_STRUCT_PTR_DEF_STRUCT(node_parser_sqlite, node_parser, string_dumper, string_dumper);
...
void node_parser_sqlite_dispose(struct node_parser* node_parser) {
    ...
    if (string_dumper != NULL) {
        ...
        MEM_STRUCT_PTR(node_parser_sqlite, node_parser, string_dumper) = NULL;
    }
}

struct node_parser* node_parser_sqlite_create(struct string_dumper* string_dumper) {
    MEM_ALLOC_STRUCT_DEF(node_parser_sqlite);
    ...
    return MEM_STRUCT_AS(node_parser, node_parser_sqlite);
}
